...
#### スクランブルタイプの変更
**スクランブルのタイプを変更するには、画面上部のスクランブルパネルからカテゴリー／サブカテゴリーのドロップダウンメニューを選択します。**スクランブルのカテゴリーを切り替えると、自動的にサブカテゴリーの内容が変更されます。スクランブルによっては、オプションを指定することができます。（例・イージークロスの手数、3x3x3複数目隠しの挑戦個数、など）
